...
| 2 | Assert | Locate the Create Mask element using its `data-test-id` value `vp-plan-preview-create-mask-btn` and verify that it is visible. | The Create Mask element is visible, confirming the plan-preview view is open. |

---

## 3. Assertions

- **Assertion 1:** The element identified by `data-test-id` value `vp-plan-preview-create-mask-btn` is visible after a voyage is opened.
