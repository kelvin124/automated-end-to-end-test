...
| 2 | Locate and click | Locate the Create Mask control using its `data-testid` value `vp-view-plan-preview-create-mask-btn`, then click it. | The application navigates to the create-mask view. |
| 3 | Assert | Verify that the text `MASK ORDER` is visible. | The create-mask view is open. |

---

## 3. Assertions

- **Assertion 1:** The text `MASK ORDER` is visible after the Create Mask control is selected.
