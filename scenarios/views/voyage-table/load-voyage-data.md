...
| 2 | Locate and validate | Locate the AG Grid voyage row-count status element using `.ag-status-bar-left[role="status"]`, because the live element does not expose a `data-test-id`. Verify that its initial text is `Rows: 0`. | The row count initially displays `Rows: 0`. |
