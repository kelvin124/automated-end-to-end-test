...
| 2 | Locate and validate | Locate the voyage row-count element using the `data-test-id` placeholder `my-test-id-voyage-row-count` until its actual value is provided. Verify that its initial text is `Rows: 0`. | The row count initially displays `Rows: 0`. |
