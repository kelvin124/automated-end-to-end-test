...
| 3 | Wait and assert | Locate the loader using its `data-testid` value `vp-loader` and wait for it to become hidden or be removed. | Create-mask table data has finished loading. |
...
- **Assertion 2:** The loader identified by `data-testid` value `vp-loader` is hidden or removed after the table data loads.
